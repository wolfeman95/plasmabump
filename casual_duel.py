...
class DuelStats:
    ...
    def populate_stats(self, match_events):
        ...
        #reverse key/value for player1/nick, player2/nick
        ...
        for event in match_events[1:-1]: #everything except header / footer
            ...
            death_event = '{}_total_deaths'.format((nick_to_player[event[2]] if event[1] == 'suicides' else nick_to_player[event[4]]))  # death total: player2_total_deaths (or player1 if suicide)
            ...
            for any in [frag_event, total_event, death_event]:
                ...
                # if any not in self.STATS:
                #     self.STATS.update({any: 1})
                # else:
                #     self.STATS.update({any: self.STATS[any] + 1})
    ...
    def post_match(self):
        print('MATCH OVER; ABOUT TO POST THIS JSON DATA --- ')
        print(self.payload_stats)
        print('=========================================================')
        try:
            #http://localhost:8000/api/duels/           http://www.plasmabump.com/api/duels/
            r = requests.post("http://www.plasmabump.com/api/duels/", data=self.payload_stats, auth=('user', 'pass'))
            r.raise_for_status()
            print("DUEL SHOULD HAVE POSTED TO DB")
            print("DUEL POST status -- " + str(r.status_code))
        except requests.exceptions.HTTPError as e:
            print("DUEL MATCH DID NOT POST IDK - ", e, "POST status", r.status_code, r.text)

        self.payload_stats.clear()

class BaseMessage:
    '''date/time of message, publish message method'''
    ...
    def parse(self, line, joined_players):
        for pattern in self.PATTERNS:
            ...
            if m is not None and m.group('player') in joined_players:
                ...
                print('removeing player from joined')
...
class MatchEndMessage(BaseMessage):
    ...
    def parse(self, line, match_events, time_start):
        ...
        #footer: [game ended, match_length, winner]
...
def stdout_reader(proc,):
    ...
    for l in iter(proc.stdout.readline, b''):
        #everything modding players_in_server / match_underway go here
        line = l.decode('utf-8')
        print('got line: {}'.format(line, end=''))
        ...
        #map vote passed
        if re.match(MapVoteMessage.PATTERNS[0], line):
            print('new map vote passed!')
            match_underway = False
        ...
        if any(re.match(pattern, line) for pattern in MatchEndMessage.PATTERNS):
            ...
            joined_players.clear()
            match_events.clear()
            print('completed match end if statement')
            # clear match_events / everything
        ...
        # player disconnect / timeout + they were part of active match
        if any(re.match(pattern, line) for pattern in PlayerDisconnectMessage.PATTERNS) and match_underway:
            players_in_server -= 1
            PlayerDisconnectMessage(match_events, line).parse(line, joined_players)
            match_underway = False
            print('player dc / timout')
        ...
        if match_underway:
            print('going to parse line')
            parse_line(match_events, line, time_start)

        print(players_in_server, " players in server, ", "joined players: ", joined_players)


def main():
    proc = subprocess.Popen(['./odasrv', '-i', '-u', '-config', 'casual_duel.cfg'], #./odasrv or odasrv.exe
